...
Pixel Image::get(int col, int row) const {
...
void Image::set(int col, int row, const Pixel& color) {
...
Image Image::resize(int width, int height) const {
   ...
   for(int i = 0; i < width; i++){
      for(int j = 0; j < height; j++){
         ...
         _i = floor((i/(width-1.0f))*(w-1.0f));
         _j = floor((j/(height-1.0f))*(h-1.0f));
...
Image Image::flipHorizontal() const {
   Image result(w, h);
   for(int i = 0; i < w ; i++){
      for(int j = 0; j < round(h/2.0f + 0.5f); j++){
         struct Pixel swap = get(i,(h-1)-j);
         result.set(i,(h-1)-j, get(i,j));
         result.set(i,j,swap);
      }
   }
